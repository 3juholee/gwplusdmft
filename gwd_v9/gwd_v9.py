...
Hyb function for DMFT is given by H_HF (static) Hamiltonian 
See the keyword "Ver9"
"""
...
def Cmp_Delta(mu,Ham, Eimp, Sigs,Edc):
	Delta=zeros(shape(Sigs),dtype=float)
	Delta[:,0] = Sigs[:,0]
	embed = dot(TProj.T,TProj)
	M=len(Ham)
	for iw,w in enumerate(Sigs[:,0]):
		Sigm = (Sigs[iw,1]+Sigs[iw,2]*1j)
		Ham_w = Ham+(Sigm-Edc)*embed
		Gw = linalg.inv(identity(M)*(w*1j+mu)-Ham_w)
		Gloc= dot(TProj,dot(Gw,TProj.T))[0,0]
		Delt = w*1j-Eimp-Sigm-1/Gloc
...
########### Initial variables (mu,G,Gloc,Nt,Eimp,Delta) ###########
	...
	if DMFT:
		...
		if os.path.exists("./Delta.dat"):
			...
			Sigs[:,0] = oms_f_log
			Edc=0.0
			Delta = Cmp_Delta(mu,Ham0,Eimp, Sigs,Edc)
...
###############self-consistent loop################
	...
	for itt in range(Nitt):
		...
		################ DMFT self-energy #########################
		if DMFT:
			#savetxt('Delta.dat',Delta)
			if abs(Delta[-1][1])>5e-4: #Re(D(oo))==0?
				print "real(Delta(oo)) is not zero, EXIT!"
				sys.exit()
		...
		if not fixmu:
			if Nfix:
				print "total N is fixed by 2.0"
				mu = Cmp_mu(Noccup/2.,mu_a,mu_b,eks_GW,oms_f_log,oms_equal_f,beta)
			else:
		...
		# New Eimp and Delta
		if DMFT: # Ver9: Ham is from HF
			Eimp0 = dot(TProj,dot(Ham,TProj.T))[0,0]-mu-Edc
			Delta0 = Cmp_Delta(mu,Ham,Eimp0, Sigs,Edc)
		#Nt1
		...
		#W1 and Wloc1
		R_w = R_svd_Gtau(Gtau, UH, UdotS_HF, UdotSi_HF ,oms_b_log)
		R_tau = Ww_to_Wtau(R_w, tau_mesh, oms_b_log, oms_b,beta)
		Pwloc = Gtau_to_Pwloc(Gloctau, tau_mesh,tau_large,oms_b_log)
		Wloc = U_local/(1-U_local*Pwloc)-U_local
		Wloc_tau = Ww_to_Wtau_loc(Wloc, tau_mesh, oms_b_log, oms_b,beta)
		...
		spectral = zeros(len(Gw0),complex)
		for iw in range(len(Gw0)):
			sp = trace(Gw0[iw])
			spectral[iw] = sp
